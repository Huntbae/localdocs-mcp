...
- **방법 1 (지금 당장, 설정 0)**: Claude 세션에 연결된 **Google Drive 커넥터**로 텍스트를 받아 인덱스에 넣습니다. 대량은 서브에이전트로 병렬 처리.
- **방법 2 (자동화, 예정)**: 도구에 `gdrive-index` 소스를 내장해 Drive API로 폴더를 훑어 텍스트만 인덱싱(1회 OAuth 필요, `modifiedTime` 증분). 로드맵 항목.
